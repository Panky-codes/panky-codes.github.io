...
But context of the problem first!
...
```
In the above code snippet, the function call to `get_display_pixels` generates a temporary that is being used to construct the `disp_pixels` variable. As the temporary is a `rvalue`, compiler is smart enough to match it with the move constructor that takes in `rvalue reference` as its input. 
## Conclusion
