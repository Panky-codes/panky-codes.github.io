...
Large block sizes(LBS) in Linux. Checkout [Part0](https://blog.pankajraghav.com/2024/03/14/LBS1.html)
...
understanding. Before reading this article, I would highly recommend
checking out [Part0](https://blog.pankajraghav.com/2024/03/14/LBS1.html)
...
This is a simplification of the patchset, but this is the main meat of the
...
been in the kernel, but many filesystems uses it. fsblock was an attempt
...
especially maximum number of buffers a `struct buffer_head` can hold is limited
by `PAGE_SIZE`.
...
most a single page.(See MAX_BUF_PER_PAGE [link](https://elixir.bootlin.com/linux/v6.9.5/source/include/linux/buffer_head.h#L43))
...
Similar to buffer_head, fsblock struct holds a disk block in memory but
...
filesystems to have LBS support.

This patchset did not get any traction, probably because it was added as
...
PAGE_SIZE support in XFS. [VFS IOMAP library](https://www.kernel.org/doc/html/next/filesystems/iomap/design.html#library-design)
...
| ![ZERO AROUND](/assets/LBS/LBS_IU.png) |
|:--:|
| Sub-block IO requiring zeroing around |

**Direct IO**: Minimal changes were required to support LBS in the
direct IO path. All that needs to be done is padding of zeroes to an IO
so that it can occupy the entire block.

**Writeback**: It is the process used by Linux to write the dirty pages
...
**Buffered IO**: IOMAP_F_ZERO_AROUND was mainly added to support
...
`struct folio` conversion around this time, and it solves this zero
around problem from the memory management layer. The final
implementation we got it upstreamed relied heavily on `struct folio`
implementation.

Even though this patchset did not upstream bs > ps support, lots of the
XFS bugs to support bs > ps were fixed as a part for this series, which
made things very easy to support bs > ps in XFS.
...
In the next post, I will cover the final LBS support that was added to
mainline Linux 6.12.
