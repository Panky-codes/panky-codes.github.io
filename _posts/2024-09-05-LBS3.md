...
the filesystem enables large folios mapping. The first filesystem to enable this was XFS. Note that this is an **optimization** based on
the size of the readahead and the memory pressure.

This support is really crucial for LBS as page cache is not any more tied
...
**iomap** already supports large folios, and it got further optimizations to
...
support when it was ported to Linux.
...
```
### Conclusion:
The presence of large folio support in the kernel greatly reduced the
complexity of adding LBS support. This work is an accumulation of
various efforts that have been made in the past [LBS part 2](https://blog.pankajraghav.com/2024/09/04/LBS2.html).

The Kernel also needs this support to enable block devices with LBA size
greater than the PAGE_SIZE as the block cache shares the same
infrastructure as the page cache for filesystems.

Enabling LBS support in filesystems requires careful evaluation, and in
some cases, filesystem changes. This
[series](https://lore.kernel.org/linux-fsdevel/20240822135018.1931258-1-kernel@pankajraghav.com/)
