...
##### **FreeRTOS Tick issue**
Even though this new subsection breaks the flow of the article, I need to give a special mention to one of those head scatching moments where I started to doubt my skill as an embedded software developer! 

I wanted to give a small delay (around 1 to 2 milliseconds)between every `Fetch-Decode-Execute` cycle. When I introduced this delay, FreeRTOS would complain . During the implementation of the `CHIP8 class` 

Some people might ask: "Well what you did is fine, but is there a diffe building a CHIP8 emulator in a microcontroller i"
